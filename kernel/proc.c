...
procinit(void)
{
  ...
  for(p = proc; p < &proc[NPROC]; p++) {
      ...
      initlock(&p->ktid_lock, "next_ktid");
      p->state = UNUSED_P;
      p->exited = 0;
      kthreadinit(p);
  }
...
myproc(void)
{
  if (mykthread()->belong != 0)
  {
    return mykthread()->belong;
  }
  else 
  {
    return 0;
...
found:
  ...
  //allocating the first kthread, using allockthread
  if(allockthread(p) == 0){
    freeproc(p);
    release(&p->lock);
    return 0;
...
fork(void)
{
  ...
  // Allocate process.
  if((np = allocproc()) == 0){ //np->lock and np->kthread[0].lock are acquired.
    return -1;
  }
...
exit(int status)
{
  ...
  if (p->exited == 0){
    ...
    release(&p->lock);
    struct kthread *t;
    for (t = p->kthread; t < &p->kthread[NKT]; t++){ //terminating all kthreads pther than the calling thread
      if (t != mykthread()){
        acquire(&t->lock);
        if (t->state == (RUNNABLE||SLEEPING||ZOMBIE)){
          release(&t->lock);
          int ret1 = kthread_kill(t->ktid);
          if (ret1 < 0){
            panic("exit: kill failed");
          }
          int status = 1;
          int ret2 = kthread_join(t->ktid, (uint64)&status); 
          if (ret2 < 0){
            panic("exit: join failed");
          }
        }
        else{
          release(&t->lock);
    ...
    sched();
    panic("zombie exit"); }//close if (!exited)
  else {
    release(&p->lock);
    struct kthread * kt = mykthread();
    acquire(&kt->lock); 
...
kill(int pid)
{
  ...
  for(p = proc; p < &proc[NPROC]; p++){
    ...
    if(p->pid == pid){
      p->killed = 1;
      struct kthread *kt; //wakeup all the process' kthreads
      for (kt = p->kthread; kt < &p->kthread[NKT]; kt++)
      {
        if (kt != mykthread()){
          acquire(&kt->lock);
